...
const MuscleGroupChart = () => {
  ...
  const totalExercises = Object.values(muscleGroupCounts).reduce((acc, count) => acc + count, 0);

  console.log('Total Exercises:', totalExercises); // debugging

  // convert counts to percentages
  const data: ChartData[] = Object.entries(muscleGroupCounts).map(([muscleGroup, count]) => ({
    name: muscleGroup,
  ...
  const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#A4DE6C', '#D075DE'];
